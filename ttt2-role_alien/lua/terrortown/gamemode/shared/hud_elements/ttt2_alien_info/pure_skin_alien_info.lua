...
if CLIENT then -- CLIENT
	...
	HUDELEMENT.alien_icon = Material("vgui/ttt/dynamic/roles/icon_alien.vtf")
	...
	function HUDELEMENT:ShouldDraw()
		...
		return (GetRoundState() == ROUND_ACTIVE and LocalPlayer():GetTeam() == TEAM_ALIEN and c:IsActive()) or HUDEditor.IsEditing
	end
	...
	function HUDELEMENT:Draw()
		...
		color.a = 175
		draw.FilteredShadowedTexture(x + 8 * self.scale, y + 5 * self.scale, 30 * self.scale, 30 * self.scale, self.alien_icon, color.a, color, self.scale)

		local amnt_print = tostring(ALIEN_DATA:GetProbedAmount()) .. " / " .. tostring(ALIEN_DATA:GetAmountToWin())
